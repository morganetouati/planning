...
function calcul_total_semaine($annee, $semaine, $id_horaire, $cjournee) {
    ...
    $tab_val = $connect->prepare('SELECT * FROM horaires WHERE semaine = :semaine AND annee = :annee');
...
function calcul_total_mois($mois, $annee, $id_horaire, $cjournee) {
    ...
    $instance = Planning_Bdd_Connect::getInstance();
    $connect = $instance->getPdo();
    $reqselect = $connect->prepare('SELECT * FROM horaires WHERE mois = :mois AND annee = :annee');
...
function calcul_total_annee($annee, $id_horaire, $cjournee) {
    ...
    $instance = Planning_Bdd_Connect::getInstance();
    $connect = $instance->getPdo();
    $selectannee = $connect->prepare('SELECT * FROM horaires WHERE annee = :annee');
...
$id = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['id_users'])));
...
$total_heure_normal = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['total_heure_normale'])));
$total_semaine = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['total_semaine'])));
$total_mois = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['total_mois'])));
$total_annee = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['total_annee'])));
$annee = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['annee'])));
$semaine = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['semaine'])));
$mois = htmlentities(htmlspecialchars(mysql_real_escape_string($_POST['mois'])));
...
$updatehoraire = $connect->prepare("UPDATE horaires SET start = :start, pause = :pause, reprise = :reprise,
